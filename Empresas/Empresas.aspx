<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Empresas.aspx.vb" Inherits="Empresas_Empresas" %>
...
<body>
                    ...
                    <div class="row">
                        <div class="col-sm">
                            ...
                            <div class="row mb-4 mt-1">
                                ...
                                <div class="col-5 text-right"><a href="NuevaEmpresa.aspx" class="btn btn-primary">+ Nueva Empresa</a></div>
                            </div>

                            <table id="tblEmpresas" class="table table-striped table-bordered dataTable hover nowrap" style="width: 100%">
                                <thead>
                                    <tr>
    ...
    <div class="modal fade" id="dlgMensajeConfirmacionModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="MensajeConfirmacionModalLabel" aria-hidden="true" style="z-index: 1800;">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button id="btnEliminarEmpresa" type="button" class="btn btn-primary mr-sm-2">Aceptar</button>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            ListaGeneralEmpresas().then(estado => {
                ...
                $('#tblEmpresas').dataTable({
                    ...
                    { "responsivePriority": 1, targets: 0 }, // Columna Razón Social
                    { "responsivePriority": 2, targets: 1 }, // Columna Representante
        ...
        //---------------------------------------------------------------------------------------------------------------------
        // Elimina el registro de datos de la empresa especificada
        //---------------------------------------------------------------------------------------------------------------------
        $("#btnEliminarEmpresa").on("click", function () {
            EliminarEmpresa(_gIdEmpresa).then(estado => {
                // Refresca página
        ...
        function fConsultar(obj) {
            ...
            window.location.href = "EditarEmpresa.aspx?Id=" + $(fila).attr('id') + "&op=1";
        }

        function fEditar(obj) {
            let fila = $(obj).parent().parent(); //Objeto fila
            //alert($(fila).attr('id')); // Id de fila
            window.location.href = "EditarEmpresa.aspx?Id=" + $(fila).attr('id') + "&op=2";
        }

        function fEliminar(obj) {
            let fila = $(obj).parent().parent(); //Objeto fila
            _gIdEmpresa = $(fila).attr('id');
            $('#dlgMensajeConfirmacionModal').modal('show');
            //$('#tblEmpresas").DataTable().ajax.reload();
        }

        function EliminarEmpresa(idEmpresa) {
            return new Promise((resolve, reject) => {
                var pageUrl = '<%=ResolveUrl("~/wsBocore.asmx")%>';
                //alert("Eliminando empresa");
                $.ajax({
                    type: "POST",
                    url: pageUrl + "/EliminarEmpresa",
        ...
        function ListaGeneralEmpresas() {
            return new Promise((resolve, reject) => {
                var pageUrl = '<%=ResolveUrl("~/wsBocore.asmx")%>';
                //alert("ListaGeneralEmpresas");
                $.ajax({
                    type: "POST",
                    url: pageUrl + "/ListaGeneralEmpresas",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    processData: false,
                    async: false, // Para usar con promesas
                    ...
                    success: function (response) {
                        ...
                        if (objResultado.Codigo == '0') {
                            $("#tbdDetalleEmpresas").empty();

                            $.each(objResultado.Resultado, function (i, usuario) {
                                let htmlTR = `<tr id="${usuario.IdEmpresa}">
                                ...
                                // Agrega una fila con información de una empresa.
                                $('#tbdDetalleEmpresas').append(htmlTR);
                            });
                            resolve(true);
                        } else {
                            reject(objResultado.Mensaje);
